...
//      dut.io.sw.poke("b000001".U)
  ...
  "_VgaSimple test2" should "pass" in {
    test(new WildcatTop("../SoC_project/out/vga_simple2.out")).withAnnotations(Seq(WriteVcdAnnotation, VerilatorBackendAnnotation, VerilatorFlags(Seq("-CFLAGS", "-std=c++14")))) { dut =>
      dut.clock.setTimeout(0)
      dut.clock.step(100)
      dut.io.sw.poke("b001000".U)
      dut.clock.step(6000)
